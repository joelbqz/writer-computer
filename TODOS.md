...
- View zoom shortcuts: [`SPECs/view-zoom-shortcuts-spec.md`](SPECs/view-zoom-shortcuts-spec.md) — Cmd+= / Cmd++ / numpad + zoom the whole window in, Cmd+- out, Cmd+0 back to actual size, through WKWebView page zoom (`getCurrentWebview().setZoom`) persisted as the global `window.zoom` percent setting (50–300, browser-style stops). Applied as a settings side effect so hydration, shortcuts, and the Preferences → Window slider share one path; palette commands Zoom In / Zoom Out / Reset Zoom; JS-only handling (no View menu needed); the Mermaid canvas now ignores Cmd chords.
