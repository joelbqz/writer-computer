...
- List item spacing — items that follow another item at the same level carry `cm-list-item-gap` on their marker line, and the theme gives it `padding-top: var(--writer-list-item-gap, 0.4em)`. First items and continuation lines are excluded; fixed value for now, no setting.
