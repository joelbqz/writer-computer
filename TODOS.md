...
- Table break-out of the editor measure (Phase B): [`SPECs/table-break-out-measure-spec.md`](SPECs/table-break-out-measure-spec.md) — a table wider than the measure now grows past it, centred, up to the pane minus a 4rem gutter that clears the section rail, and scrolls inside `.cm-table-inner` past that so the document never scrolls sideways. Tables that already fit are untouched, and a touched table still unfolds to source lines inside the measure. The 48ch cell cap was re-measured and kept: it is the largest cap at which every fixture table reaches its natural width without scrolling. Break-out also cut heightmap drift from 944px/425px to 344px/130px. Phase C (JS column-width computation) remains open, as does the pre-existing `appearance.editor-width` bug the measurements surfaced (`--writer-editor-outer-width` is composed on `:root`, so the per-pane `--writer-editor-max-width` override is inert).
