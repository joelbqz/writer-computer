...
What still shifts: a line is rebuilt with a fresh estimate when its height-relevant decorations (replace ranges, block widgets) change, and viewport-scoped plugins drop their decorations for lines leaving the viewport. Content above the visible area then moves by the estimate error (a note's first H1: 92px measured, 29px estimated). CodeMirror's measure-loop anchoring re-scrolls the ancestor scroller to hide that, but only while the editor has focus or within 100ms of a wheel or touch event; a scrollbar drag in a blurred editor shows the shift. To diagnose height churn, diff `view.lineBlockAt(line.from).height` for every line across a scroll step: lines far above the viewport changing height is the tell, and the playground exposes the view as `window.__view` for exactly this.
